...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { nodes, edges } = await getGraph(limit);
    res.json({ nodes, edges, counts: await appState.codeGraph.counts() });
  })
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const matches = await findSymbol(name, { exact });
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [symbols, dependencies, dependents, wiki] = await Promise.all([
      symbolsIn(relPath),
      dependenciesOf(relPath),
      dependentsOf(relPath),
      getWiki(relPath),
    ]);
    res.json({ relPath, symbols, dependencies, dependents, wiki });
  })
);
